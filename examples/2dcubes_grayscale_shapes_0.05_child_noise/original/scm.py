    ...
    lambda scene: {
        ...
        "obj_1": (
            ...
            lambda weight_1: scene.create_primitive(
                PrimitiveShape.CUBE,
                scale=np.repeat(OBJECT_SCALE, 3),
            ),
            None,
        ),
        "pos_x1": (
            ...
            lambda noise, obj_1: scene.set_object_position(
                obj_1, [0, noise[0] * NOISE_SCALE1, None]
            )[1],
            Uniform("P", -1, 1),
        ),
        "pos_y1": (
            ["obj_1"],
            lambda noise, obj_1: scene.set_object_position(
                obj_1, [0, None, noise[0] * NOISE_SCALE1]
            )[2],
            Uniform("P", -1, 1),
        ),
        # "weight_2": (
        ...
        "pos_x2": (
            ...
            lambda noise, obj_2: scene.set_object_position(
                obj_2, [0, noise[0] * NOISE_SCALE2, None]
            )[1],
            Uniform("P", -1, 1),
        ),
        "pos_y2": (
            ["obj_2"],
            lambda noise, obj_2: scene.set_object_position(
                obj_2, [0, None, noise[0] * NOISE_SCALE2]
            )[2],
            Uniform("P", -1, 1),
        ),
        "obj_3": (
